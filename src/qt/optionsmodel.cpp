...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBeaconAmount"))
        settings.setValue("nAnonymizeBeaconAmount", 1000);

    nObfuscationRounds = settings.value("nObfuscationRounds").toLongLong();
    nAnonymizeBeaconAmount = settings.value("nAnonymizeBeaconAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBeaconAmount"))
        SoftSetArg("-anonymizebeaconamount", settings.value("nAnonymizeBeaconAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in beacon.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBeaconAmount:
            return QVariant(nAnonymizeBeaconAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case ObfuscationRounds:
            ...
            break;
        case AnonymizeBeaconAmount:
            nAnonymizeBeaconAmount = value.toInt();
            settings.setValue("nAnonymizeBeaconAmount", nAnonymizeBeaconAmount);
            emit anonymizeBeaconAmountChanged(nAnonymizeBeaconAmount);
            break;
        case CoinControlFeatures:

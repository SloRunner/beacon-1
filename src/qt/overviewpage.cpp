...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeBeaconAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBeaconAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeBeaconAmount = strAnonymizeBeaconAmount.remove(strAnonymizeBeaconAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBeaconAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeBeaconAmount * COIN) nMaxToAnonymize = nAnonymizeBeaconAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeBeaconAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeBeaconAmount));
        strAnonymizeBeaconAmount = strAnonymizeBeaconAmount.remove(strAnonymizeBeaconAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBeaconAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
